...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('[PROCESS ACCEPTANCE V3] Starting for suggestion:', suggestionId, 'at', new Date().toISOString());
    ...
    if (!suggestion || !document) {
      console.error('[PROCESS ACCEPTANCE V3] Suggestion or document not found');
      return Response.json({ error: 'Not found' }, { status: 404 });
    }

    if (suggestion.status !== 'pending') {
      console.log('[PROCESS ACCEPTANCE V3] Already processed, skipping');
    ...
    if (!canForceAccept) {
      ...
      if (verifyDelta < verifyThreshold) {
        console.log('[PROCESS ACCEPTANCE V3] Suggestion no longer meets threshold, aborting. delta:', verifyDelta, 'threshold:', verifyThreshold);
    ...
    if (forceReleaseLock) {
      console.log('[PROCESS ACCEPTANCE V3] forceReleaseLock=true, releasing any stuck lock for', suggestionId);
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      ...
      if (lockResult && lockResult.updated === 1) {
        lockAcquired = true;
        console.log('[PROCESS ACCEPTANCE V3] Lock acquired on attempt', attempt);
        break;
      }
      if (attempt === 1) {
        ...
        if (lockCheck && lockCheck.status === 'pending' && lockCheck.acceptanceLock === true) {
          ...
          console.log('[PROCESS ACCEPTANCE V3] Lock held (age:', Math.round(lockAgeMs / 1000) + 's) on attempt 1');
          if (lockAgeMs > 90000 || forceReleaseLock) {
            console.log('[PROCESS ACCEPTANCE V3] Force-releasing stale lock for', suggestionId);
    ...
    if (!lockAcquired) {
      console.log('[PROCESS ACCEPTANCE V3] Lock not acquired after retry, another instance holds it');
    ...
    try {
      ...
      console.error('[PROCESS ACCEPTANCE V3] active voter count failed, skipping cap:', e);
    }
    ...
    console.log('[PROCESS ACCEPTANCE V3] Calculated:', { totalUsers, activeVoterCount, boundedConsensus, rawThreshold, cap, newThreshold });
    ...
    if (suggestion.type === 'edit_section' && suggestion.sectionId) {
      ...
      if (!section) {
        ...
        if (!resurrectTopicId) {
          console.error('[PROCESS ACCEPTANCE V3] Cannot resurrect — no topicId on suggestion');
        ...
        if (pendingResurrectChildren.length > 0) {
          console.log('[PROCESS ACCEPTANCE V3] Re-linking', pendingResurrectChildren.length, 'child suggestions to resurrected section', section.id);
        ...
        if (otherSlotOrphans.length > 0) {
          console.log('[PROCESS ACCEPTANCE V3] Re-linking', otherSlotOrphans.length, 'orphaned suggestions to resurrected section', section.id);
        ...
        console.log('[PROCESS ACCEPTANCE V3] Resurrected deleted section', section.id, 'at order', resurrectOrder);
      ...
      if (suggestion.sectionId) {
        console.log('[PROCESS ACCEPTANCE V3] new_section already has sectionId', suggestion.sectionId, '— skipping section creation (retry recovery)');
      } else {
      ...
      if (!targetTopicId) {
        console.error('[PROCESS ACCEPTANCE V3] No targetTopicId for new_section — aborting without accepting');
      ...
      if (suggestion.insertPosition !== undefined && suggestion.insertPosition !== null) {
        ...
        if (sectionsToShift.length > 0) {
          ...
          if (suggsToShift.length > 0) {
            ...
            console.log('[PROCESS ACCEPTANCE V3] Shifted insertPosition for', suggsToShift.length, 'pending suggestions');
          }
        }
      } else {
        newOrder = maxOrder + 1;
      ...
      if (pendingChildren.length > 0) {
        console.log('[PROCESS ACCEPTANCE V3] Re-linking', pendingChildren.length, 'child suggestions to new section', newSection.id);
      ...
      if (parentSuggestion) {
        ...
        if (parentSuggestion.status === 'pending') {
          ...
          if (parentDelta >= parentThreshold) {
            console.log('[PROCESS ACCEPTANCE V3] Parent still pending AND meets threshold, triggering its acceptance:', suggestion.parentSuggestionId, { parentDelta, parentThreshold });
            try {
              await base44.asServiceRole.functions.invoke('processAcceptanceV3', {
              ...
              console.log('[PROCESS ACCEPTANCE V3] Parent suggestion processed successfully');
            } catch (parentErr) {
              console.error('[PROCESS ACCEPTANCE V3] Failed to process parent suggestion:', parentErr);
            }
          } else {
            console.log('[PROCESS ACCEPTANCE V3] Parent still pending but below threshold — updated content only, NOT auto-accepting parent:', suggestion.parentSuggestionId, { parentDelta, parentThreshold });
          }
        }
      } else {
        console.warn('[PROCESS ACCEPTANCE V3] Parent suggestion not found:', suggestion.parentSuggestionId);
      }
      ...
      if (section) {
        ...
        try {
          ...
          if (orphaned.length > 0) {
            ...
            console.log('[PROCESS ACCEPTANCE V3] Anchored', orphaned.length, 'orphaned suggestions to original position');
          }
        } catch (orphanErr) {
          console.error('[PROCESS ACCEPTANCE V3] Failed to anchor orphaned suggestions:', orphanErr);
        }
    ...
    if (notifications.length > 0) {
      console.log('[PROCESS ACCEPTANCE V3] Sending', notifications.length, 'notifications...');
      try {
        await base44.asServiceRole.entities.Notification.bulkCreate(notifications);
        console.log('[PROCESS ACCEPTANCE V3] ✓ Sent successfully');
      } catch (err) {
        console.error('[PROCESS ACCEPTANCE V3] Failed:', err);
      }
    }

    console.log('[PROCESS ACCEPTANCE V3] Completed successfully');
    ...
    console.error('[PROCESS ACCEPTANCE V3 ERROR]', error);
    ...
    if (lockAcquired && base44 && suggestionId) {
      try {
        ...
        console.log('[PROCESS ACCEPTANCE V3] Released acceptance lock after failure for', suggestionId);
      } catch (unlockError) {
        console.error('[PROCESS ACCEPTANCE V3] Failed to release acceptance lock:', unlockError);
      }
    }
