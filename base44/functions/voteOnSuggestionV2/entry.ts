...
Deno.serve(async (req) => {
  try {
    ...
    console.log('[VOTE V2 FUNCTION] Processing vote:', { suggestionId, vote, userId: user.id });
    ...
    if (allUserVotes.length > 1) {
      console.warn('[VOTE V2 FUNCTION] Found', allUserVotes.length, 'votes for user, cleaning up duplicates');
    ...
    freshVotes.forEach(v => {
      if (votesByUser.has(v.userId)) {
        console.warn('[VOTE V2 FUNCTION] DUPLICATE VOTE DETECTED for userId:', v.userId, 'on suggestion:', suggestionId);
    ...
    if (duplicateVoteIds.length > 0) {
      console.log('[VOTE V2 FUNCTION] Cleaning up', duplicateVoteIds.length, 'duplicate votes');
    ...
    console.log('[VOTE V2 FUNCTION] Vote processed:', { voteAction, newProVotes, newConVotes });
    ...
    if (voteAction === 'created') {
      try {
        ...
        console.log('[VOTE V2 FUNCTION] Updated totalUsersInteracted:', totalUsers);
      } catch (e) {
        console.error('[VOTE V2 FUNCTION totalUsersInteracted update error]', e);
      }
    }
    ...
    console.log('[VOTE V2 FUNCTION] Consensus check:', { delta, threshold, shouldAccept });
    ...
    if (shouldAccept && suggestion.status === 'pending') {
      ...
      if (processingAcceptance.has(lockKey)) {
        console.log('[VOTE V2 FUNCTION] Already processing acceptance, skipping');
        return Response.json({ 
          success: true, 
          newProVotes, 
          newConVotes, 
      ...
      let processAcceptanceDebug = null;
      try {
        const acceptResult = await base44.asServiceRole.functions.invoke('processAcceptanceV4', {
          suggestionId,
        ...
        console.error('[VOTE V2 FUNCTION] processAcceptanceV4 error:', err, processAcceptanceDebug);
      } finally {
        processingAcceptance.delete(lockKey);
      }

      // Verify against the database — the single source of truth
      ...
      if (!accepted) {
        console.error('[VOTE V2 FUNCTION] Acceptance did not happen. Debug info:', {
