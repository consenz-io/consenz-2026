...
${rejected.map(s => `  • ${fmtSuggestionFull(s)}`).join('\n') || '  (none)'}
...
4. Highlight open suggestions with their direct links so readers can click and vote easily. When listing suggestions (whether open, accepted, or rejected), describe the suggestion content itself—not just the topic it belongs to.
