...
Deno.serve(async (req) => {
  try {
    ...
    const [document, suggestions, sections, topics, comments, votes, agreements] = await Promise.all([
    ...
    // Filter comments relevant to this document
    ...
    // Filter votes relevant to this document
    const relevantVotes = votes.filter(v => suggestionIds.has(v.suggestionId));
