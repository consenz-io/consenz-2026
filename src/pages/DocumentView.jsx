...
export default function DocumentView() {
  ...
  // Count unique versions (by suggestionId)
  const uniqueVersionsCount = React.useMemo(() => {
    if (!documentVersions || documentVersions.length === 0) return 0;
    const uniqueSuggestions = new Set(
      documentVersions
        .filter(v => v.suggestionId)
        .map(v => v.suggestionId)
    );
    return uniqueSuggestions.size;
  }, [documentVersions]);
      ...
      <div className="max-w-6xl mx-auto space-y-2 md:space-y-6 px-1 md:px-4 w-full max-w-full">
        ...
        <div className="grid grid-cols-4 gap-2 md:gap-4 w-full max-w-full">
          ...
          <Link 
            ...
            aria-label={`${uniqueVersionsCount} ${language === 'he' ? 'גרסאות קודמות' : language === 'ar' ? 'الإصدارات السابقة' : 'versions'}. ${language === 'he' ? 'לחץ לצפייה בהיסטוריה' : 'Click to view history'}`}
          >
            <Clock className="w-4 h-4 md:w-6 md:h-6 text-teal-600" aria-hidden="true" />
            <div className="text-base md:text-xl font-bold text-slate-900">{uniqueVersionsCount}</div>
