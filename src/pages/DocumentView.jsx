...
export default function DocumentView() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-2 md:p-6 overflow-x-hidden w-full">
      <div className="max-w-6xl mx-auto space-y-3 md:space-y-6 w-full overflow-x-hidden">
        <div className="flex flex-col gap-2 md:gap-4 w-full">
          <div className={`flex items-center justify-between gap-2 w-full ${isRTL ? 'flex-row-reverse' : ''}`}>
            <h1 className="text-lg md:text-3xl font-bold text-slate-900 flex-1 min-w-0 break-words leading-tight">
              {(() => {
              ...
              <div className="flex-shrink-0">
                {isTranslating ? (
                  ...
                  <button
                    ...
                    title={t('translate')}
                  >
                    <Languages className="w-4 h-4 md:w-5 md:h-5 text-blue-600" />
                  </button>
                ) : (
                  <button
                    onClick={() => setShowTranslated(!showTranslated)}
                    className="p-1 md:p-1.5 hover:bg-slate-100 rounded transition-colors"
                    title={showTranslated ? t('showOriginal') : t('showTranslation')}
                  >
                    <Languages className={`w-4 h-4 md:w-5 md:h-5 ${showTranslated ? 'text-slate-600' : 'text-blue-600'}`} />
                  </button>
                )}
              </div>
            )}
          </div>
          <div className="flex gap-1.5 md:gap-2 flex-wrap justify-center w-full">
            <Badge variant="outline" className={`text-[9px] md:text-xs px-2 py-0.5 ${
          ...
          <div className="flex gap-1 md:gap-2 flex-wrap justify-center w-full">
            <Link to={`${createPageUrl("DocumentVersions")}?id=${documentId}`} className="flex-shrink-0">
              <Button variant="outline" size="sm" className="text-[10px] md:text-sm px-2 md:px-4 h-7 md:h-9">
