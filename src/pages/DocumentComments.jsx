      ...
      {!collapsed && (
        <div className="px-5 py-5">
          ...
          {group.type === 'suggestion' && group.suggestion && (
            <div className="rounded-lg border border-amber-100 bg-amber-50/40 mb-5 overflow-hidden">
              {/* Header */}
              <div className={`flex items-center gap-2 px-3 py-2.5 border-b border-amber-100 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <MessageSquare className="w-3.5 h-3.5 text-amber-400 flex-shrink-0" />
                <span className="text-xs font-semibold text-amber-500 uppercase tracking-wider flex-1" style={{ textAlign: 'start' }}>
                  {language === 'he' ? 'הצעה' : language === 'ar' ? 'اقتراح' : 'Suggestion'}
                </span>
                <Link
                  to={`${createPageUrl("SuggestionDetail")}?id=${group.entityId}`}
                  className={`flex items-center gap-1 text-xs text-amber-600 hover:text-amber-800 font-medium transition-colors flex-shrink-0 ${isRTL ? 'flex-row-reverse' : ''}`}
                >
                  <ExternalLink className="w-3 h-3" />
                  {language === 'he' ? 'עבור' : language === 'ar' ? 'انتقل' : 'Go'}
                </Link>
              ...
              <div className="px-3 py-3">
                <p className="text-base font-medium text-amber-800 leading-snug mb-1" style={{ textAlign: 'start' }}>
                  {group.suggestion.title}
                </p>
                {group.suggestion.explanation && (
                  <p className="text-base text-amber-700 leading-relaxed" style={{ textAlign: 'start' }}>
                    {stripHtml(group.suggestion.explanation)}
                  </p>
                )}
              </div>
            </div>
