...
export default function SuggestionSidebar({ 
  ...
  const voteMutation = useMutation({
    mutationFn: async (vote) => {
      ...
      const { shouldAccept } = await checkSuggestionConsensus(updatedSuggestion, document || parentDocument);
      if (shouldAccept && suggestion.status === 'pending') {
        await autoAcceptSuggestion(updatedSuggestion, user.id, document || parentDocument);
