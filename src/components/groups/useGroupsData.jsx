...
export function useGroupsData() {
  ...
  const { data: groupAllComments = [] } = useQuery({
    queryKey: ['groupsPageAllComments', docIds],
    queryFn: () => base44.entities.Comment.filter({ rootEntityId: { $in: docIds } }, null, 500),
    enabled: docIds.length > 0,
    placeholderData: [],
    staleTime: 5 * 60 * 1000,
    gcTime: 15 * 60 * 1000,
  ...
  const getParticipantCount = (groupId) =>
    calcGroupParticipants(groupId, groupMembers, documents, groupAllSuggestions, groupAllVotes, groupAllComments, publicProfiles);
